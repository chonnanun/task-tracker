        ...
        <div class="row">
            <div class="col-xs-3">
                ...
                <div class="form-group">
                    ...
                    <a href="#" target="_blank" class="dt-button excel-report" data-cat="3" style="vertical-align:middle;">Cat Excel Report</a>
                </div>
 ...
 <?php //print_r($customfields); ?>
        ...
        <div class="row allbox boxen">
          ...
          <!--                   งานทั้งหมด -->
                    ...
                    <table id="jsontable" class="table table-hover display dataTable no-footer" cellspacing="0" width="2000">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Title / HWB</th>
                                ...
                                <?php
                                $tb_fields_tmp = array();
                                $tb_fields_tmp[] = array('data'=>'id');
                                $tb_fields_tmp[] = array('data'=>'title');
                                $tb_fields_tmp[] = array('data'=>'description');
                                $tb_fields_tmp[] = array('data'=>'created_at');
                                $tb_fields_tmp[] = array('data'=>'last_updated');
                                $tb_fields_tmp[] = array('data'=>'cat_name');
                                $tb_fields_tmp[] = array('data'=>'assigned');
                                
                                foreach($customfileds_head as $field )
                                { 
                                    $tb_fields_tmp[] = array('data'=>$field);
                                    ?>
                                    <th class="custcols cust-<?= $field ?>"><?= $function->showKey($field) ?></th>
                                    <?php
                                }
                                $tb_columns = json_encode($tb_fields_tmp);
                                ?>
...
<script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            var table = $('#jsontable').DataTable( {
                ...
                "ajax": '<?php echo base_url(); ?>json/list-<?= $rand ?>.json',
                "columns": <?php echo $tb_columns ?>,
                "columnDefs": [
                    {
                        "render": function ( data, type, row ) {
                            console.log(row);
                            return '<a href="#" onclick="linkedit('+row.id+');">' +data +'</a>';
                    ...
                    {
                        "render": function ( data, type, row ) {
                            return '<span class="colorbar-default">'+row.statusName+' </span>' +data ;
